...
export class FileService {
    constructor() {
        this.MAX_COMPRESSED_SIZE = 300 * 1024;
        this.uploadDir = process.env.UPLOADS_DIR || path.join(__dirname, '/app/uploads/');
        this.avatarDir = process.env.AVATAR_DIR_PATH || path.join(this.uploadDir, 'avatars/');
    }
    // --------------------------------------------------------------------------------- //
    async normalizeAvatar(buffer) {
        try {
    ...
    // --------------------------------------------------------------------------------- //
    async uploadAvatarSafe(buffer, userId) {
