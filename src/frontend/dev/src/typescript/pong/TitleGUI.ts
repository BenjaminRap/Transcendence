export class	TitleGUI extends HTMLElement
{
	...
	connectedCallback()
	{
		...
		this.innerHTML = `
			<p class="absolute text-[15vw] top-0 left-(--title-left) text-(--title-color) -translate-x-1/2 font-(family-name:--font) text-shadow-(--title-shadow) transition-all duration-[1s]">PONG</p>
		`;
	}
}
