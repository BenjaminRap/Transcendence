	...
	connectedCallback()
	{
		...
		this.innerHTML = `
			<div class="flex flex-col size-full h-4/6 w-1/4 left-1/2 -translate-1/2 top-1/2 absolute">
				${this.getButtonHTML("2 vs 2", "twoVersusTwo")}
				${this.getButtonHTML("Tournament", "tournament")}
				${this.getButtonHTML("Cancel", "cancel")}
			</div>
		`;
		this._inputs = {
			twoVersusTwo: this.querySelector("button.twoVersusTwo")!,
			tournament: this.querySelector("button.tournament")!,
			cancel: this.querySelector("button.cancel")!
		}
	}
