...
export class	MenuGUI extends HTMLElement
{
	...
	{
		const	buttonLeft = this.querySelector(`button#${baseId}ButtonLeft`)!;
		const	buttonRight = this.querySelector(`button#${baseId}ButtonRight`)!;
	...
	private	getPlayButton(id : string)
	{
		return `<button id="${id}" class="absolute bottom-0 left-1/2 -translate-1/2 h-1/12 w-1/4 text-[3vw] pointer-events-auto menu-button">Play</button>`;
	}
	...
	private	getItemSwitchElement(idBase : string, items : string[])
	{
		...
		return `
			...
			<div class="relative h-2/3 grow-2 text-[2vw] mx-[5%] overflow-hidden menu-button" >
				<div id="${idBase}ItemsContainer" class="size-full absolute transition-all ease-in-out left-0">
					${itemsHTML}
				</div>
			</div>
			${this.getButtonHTML(`${idBase}ButtonRight`, "right")}
		`;
	}

	private	getButtonHTML(id : string, side : "left" | "right") : string
	{
		const	rotate = (side == "right") ? "rotate-y-180" : "";

		return  `<button id="${id}" alt="" class="h-full w-[14%] font-(family-name:--font) hover:scale-(--hover-scale) hover:brightness-(--hover-brightness) ${rotate} active:scale-(--active-scale) active:brightness-(--active-brightness) pointer-events-auto bg-(image:--switch-button-image) transition-all bg-contain bg-no-repeat bg-center hover:bg-(image:--switch-button-image-hover) hover:animate-(--hover-animation)">
