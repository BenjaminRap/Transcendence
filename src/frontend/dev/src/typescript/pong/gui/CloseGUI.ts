...
export class	CloseGUI extends HTMLElement implements IGUI<CloseGUIInputs>
{
	...
	constructor()
	{
		...
		this.innerHTML = `
			<button class="aspect-square w-[3%] closeGUIClose bg-(--background-color) bg-(image:--background-color) border-(--border-color) border-(length:--border-width) rounded-[20%] absolute right-[0.5cqw] top-[0.5cqw] pointer-events-auto hover:not-disabled:scale-(--hover-scale)">
